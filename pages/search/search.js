...
Page({
  ...
  typing(e) {
    // 判断输入框的清空按钮是否需要隐藏
    if (!e.detail.value && this.data.isClearInputShow == true) {
      this.setData({
        isClearInputShow: false
      })
    } else if (e.detail.value.trim() !== '' && this.data.isClearInputShow == false) {
      this.setData({
        isClearInputShow: true
      })
    }
